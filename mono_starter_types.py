# This program is designed to find new options for starter pokemon types. Excludes water,fire,grass from options.
...
class Type:
    ...
    def attack(self, pokemon):
        # All attacks are a single type; type-effectiveness of an attack is
...
# All types in alphabetical order. Lists are empty and filled later because they reference each other
Bug = Type("Bug", [], [], [])
Dark = Type("Dark", [], [], [])
Dragon = Type("Dragon", [], [], [])
...
# type_list_trad = [Water, Fire, Grass]
type_list = [
    Water, Fire, Grass, Bug, Dark, Dragon, Electric, Fairy, Fighting, Flying,
...
multiple_immunity_list = one_immunity_list+two_immunity_list
immunity_lists = [no_immunity_list, multiple_immunity_list]
...
for trio_list in effectiveness_lists:
    if trio_list is all_se:
        cprint("SE - SE", "blue", None, [])
    elif trio_list is all_neu:
        cprint("NEU - NEU", "blue",  None, [])
    elif trio_list is se_neu:
        cprint("SE - NEU", "blue",  None, [])
    elif trio_list is se_nve:
        cprint("SE - NVE", "blue",  None, [])
    elif trio_list is nve_neu:
        cprint("NVE - NEU", "blue",  None, [])
    ...
    for imm in immunity_lists:
        ...
        # printed variable ensures layer is displayed only once for all relevant trios
        printed = False
        ...
        for self_relation in self_relationships_lists+[no_self_relation]:
            ...
            if current_list:
                ...
                for trio in current_list:
                    ...
                    # display the trio
                    cprint(f"{current_list.index(trio) +1}: {trio}",
